...
![Untitled](/Users/moeyg/Desktop/Front-end-Knowledge/Images/Browser-Rendering/Browser-Rendering-4.png)
<br>
...
우선 DOM 트리는 완전히 구문 분석된 HTML 페이지의 Object 표현입니다.

루트 요소 <html> 태그로 시작하여 페이지의 각 element / text 에 대한 노드가 만들어집니다. 다른 요cd소 내에 중첩된 요소는 자식 노드로 표시되며 각 노드에는 해당 요소의 전체 특성이 포함된다.

다음으로 CSSOM은 DOM과 비슷한 방식으로 표현되지만, 명시적 또는 암시적 선언과 상속 여부에 관계없이 각 노드의 관련 스타일로 표시됩니다.

참고로 CSS는 리소스를 완전히 파싱 하지 않으면, 렌더트리를 구성 할 수 없습니다. CSS는 HTML과 달리 계단식 상속 특성 때문에 부분적으로 실행될 수 없습니다.

다음으로 JS를 실행합니다.

렌더트리는 DOM과 CSSOM의 조합이다. 페이지에서 최종적으로 렌더링 될 내용을 나타내는 트리입니다.

즉, 표시되는 내용만 캡쳐하기 때문에 `display:none`을 사용하여 CSS로 숨겨진 요소는 포함하지 않습니다.

레이아웃은 뷰포트의 크기에 관련된 CSS 스타일에 대한 텍스트에 의해 뷰포트의 크기를 결정합니다.

마지막으로 페인팅 단계에서 페이지의 가시적인 내용을 픽셀로 변환하여 화면에 표시 할 수 있습니다. 페인트 단계에서 처리에 걸리는 시간은 DOM의 크기와 적용되는 스타일에 따라 다릅니다.

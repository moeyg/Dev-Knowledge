...
## 정리
 ...
 JS는 **싱글 스레드 기반 언어**로, 특정 코드를 수행 완료한 이후 아래줄의 코드를 수행하는 동기적 처리 방식을 갖습니다. 따라서 JS 엔진은 단일 콜 스택을 갖습니다. 

  콜 스택의 역할은 실행되는 순서를 기억합니다. 함수를 실행하려면, 스택에 해당하는 함수를 집어 넣고, 리턴이 일어나면, 스택의 가장 위쪽에서 해당하는 함수를 꺼내줍니다. 이것이 콜 스택이 하는 일의 전부입니다.

 이렇게 순차적으로 하나씩 실행이 된다면, 무거운 함수가 있을 경우에는 속도가 저하되는 상황이 발생할 것입니다. 그렇기 때문에 콜백을 받아 비동기적 처리 방식을 사용하여 효율적으로 처리할 수 있습니다.

 비동기 요청 처리는 JS를 실행하는 환경인 브라우저나 Node.js가 담당하게 됩니다. **여기서 JS 엔진과 그 실행 환경을 상호 연동시켜주는 장치가 바로 이벤트 루프입니다.**

 메시지 큐란 데이터를 서로 교환할 때 사용하는 통신 방법입니다. 즉, 비동기 메시지를 사용하는 응용 프로그램 간의 데이터 송수신을 의미합니다. 이를 통해 나중에 처리하는게 가능해 집니다.

 JS의 실행 환경은 태스크 큐와 마이크로 태스크 큐, 두 가지를 가지고 있습니다. 태스크가 콜백 함수라면 그 종류에 따라 다른 큐에 담깁니다.
 ...
 따라서 이벤트 루프를 통해 비동기적 과정처럼 효율적인 실행이 가능하도록 합니다.
